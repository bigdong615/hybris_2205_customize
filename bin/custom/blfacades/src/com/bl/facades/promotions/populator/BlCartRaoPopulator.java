...
public class BlCartRaoPopulator implements Populator<AbstractOrderModel, CartRAO> {
  ...
  public void populate(final AbstractOrderModel source, final CartRAO target)
  {
    ...
    if(BlRentalDateUtils.getRentalsDuration() != null) {
      target.setRentalDurationDays(BlRentalDateUtils.getRentalsDuration().getSelectedDays()!= null ? (source.getIsExtendedOrder() && source
          .getTotalExtendDays() > 0 ? source
          .getTotalExtendDays() : Integer.valueOf(BlRentalDateUtils.getRentalsDuration().getSelectedDays())): 7);
