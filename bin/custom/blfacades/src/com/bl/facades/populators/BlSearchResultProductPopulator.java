...
public class BlSearchResultProductPopulator implements Populator<SearchResultValueData, ProductData>
{
	...
	public void populate(final SearchResultValueData source, final ProductData target)
	{
		...
		//Reading category/subcategory data for tealium script
		try
		{
			final Collection categoryNames = this.<Collection> getValue(source, "categoryName");
			...
			categoryNames.forEach(category -> {
				...
				if (category != null)
				{
					categoryData.setCode(category.toString());
	...
	private void populateSerialPromotionMessage(final ProductData target)
	{
		...
		if (baseStore != null && StringUtils.isNotBlank(baseStore.getUsedGearPromotionMessage())
				&& getBlPromotionService().isUsedGearCategoryPromotionActive())
		{
	...
	protected void populatePrices(final SearchResultValueData source, final ProductData target)
	{
		...
		if (priceValue != null)
		{
			BlLogger.logFormatMessageInfo(LOG, Level.DEBUG, "Default Price Value is {} for Product : {}", priceValue,
					target.getCode());
			...
			BlLogger.logFormatMessageInfo(LOG, Level.DEBUG, "Dynamic Calculated Price Value is {} for Product : {}",
					dynamicPriceValue, target.getCode());
		...
		{
			final BlProductModel blProductModel = getProductForPK(target.getPrimaryKey());
			final BigDecimal dynamicPriceValue = getCommercePriceService().getDynamicPriceDataForBundleProduct(constrained,
					blProductModel);
			BlLogger.logFormatMessageInfo(LOG, Level.DEBUG, "Dynamic Calculated Price Value is {} for Product : {}",
					dynamicPriceValue, target.getCode());
			target.setPrice(Objects.nonNull(dynamicPriceValue) ? getProductPriceData(dynamicPriceValue)
					: (getProductPriceData(BigDecimal.valueOf(0.0d))));
		}
	}
				...
				if (!blProductService.isAquatechProduct(blProduct))
				{
					...
					if (Objects.nonNull(rentalDatesFromSession) && Objects.nonNull(blProductModel))
					{
						final String nextAvailableDate = getBlCommerceStockService()
								.getNextAvailableDateForSelectedDuration(blProductModel.getCode(), 1, rentalDatesFromSession);
	 ...
	 *           target to be fill
	 * @param key
	 *           key to fetch value from source
	 * @param value
	 *           value to be set for target
	 */
	private void setProductTagValues(final SearchResultValueData source, final ProductData target, final String key,
			final String value)
	{
		if (StringUtils.isBlank(target.getProductTagValues()) && null != this.<Boolean> getValue(source, key)
				&& this.<Boolean> getValue(source, key).booleanValue())
		{
	...
	private void setUpcomingAttributeValue(final SearchResultValueData source, final ProductData target, final String key)
	{
		...
		if (null != this.<Boolean> getValue(source, key) && this.<Boolean> getValue(source, key)
				&& !blProductService.isAquatechProduct(blProductModel))
		{
	...
	public void setBlWishlistOptionsPopulator(final BlWishlistOptionsPopulator blWishlistOptionsPopulator)
	{
	...
	public void setBlPromotionService(final BlPromotionService blPromotionService)
	{
