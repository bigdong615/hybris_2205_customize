...
public class BlDomoCartPopulator extends BlCartPopulator
{
	...
	public void populate(final CartModel source, final CartData target)
	{
		try
		{
			...
			target.setDepositPaymentTransactions(source.getDepositPaymentTransactions().stream()
					.map(PaymentTransactionModel::getCode).collect(Collectors.joining(", ")));
		...
		{
			LOG.error("Error while getting cart info for PK " + source.getPk().toString());
			BlLogger.logMessage(LOG, Level.ERROR, StringUtils.EMPTY, "Error while getting cart", exception);
			exception.printStackTrace();
			getDefaultBlDomoFailureNotificationService().send(exception.toString(), source.getPk().toString(), "Cart api");
		}
	}
