...
public class DomoAddressPopulator extends BlAddressPopulator
{
	...
	public void populate(final AddressModel source, final AddressData target)
	{
		try
		{
			...
			{
				target.setOwnerPk(source.getOwner().toString());
			}
		}
		catch (final Exception exception)
		{
			getDefaultBlDomoFailureNotificationService().send(exception.toString(), source.getPk().toString(), "Address api");
			LOG.debug("Error while getting Address for PK " + source.getPk().toString());
