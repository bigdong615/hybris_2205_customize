...
public class CustomerProfileDataPopulator implements Populator<CustomerModel, CustomerProfileData>
{
	 ...
	 */
	@Override
	public void populate(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		populateDeliveryAddress(userModel, customerProfileData);

		populateBillingAddress(userModel, customerProfileData);

		customerProfileData.setPaymentInfoList(getPaymentInfoList(userModel));
	}

	private void populateBillingAddress(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		...
		{
			customerProfileData
					.setBillingAddress(getAddressConverter().convert(userModel.getDefaultPaymentInfo().getBillingAddress()));
		...
		{
			final Optional<PaymentInfoModel> optionalPayment = userModel.getPaymentInfos().stream().findFirst();
			if (optionalPayment.isPresent())
			{
				final PaymentInfoModel paymentInfoModel = optionalPayment.get();
	...
	private void populateDeliveryAddress(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		...
		{
			final Optional<AddressModel> optionalAddress = userModel.getAddresses().stream().findFirst();
			if (optionalAddress.isPresent())
			{
				final AddressModel addressModel = optionalAddress.get();
