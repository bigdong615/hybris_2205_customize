...
public class AbstractBlCategoryPageController extends AbstractCategoryPageController
{
	...
	protected String performSearchAndGetResultsPage(final String categoryCode, String searchQuery, final int page, //NOSONAR
			final ShowMode showMode, final String sortCode, final Model model, final Map<Object, Object> requestAndResponseMap)
			throws UnsupportedEncodingException
	{
		...
		final String redirection = checkRequestUrl((HttpServletRequest) requestAndResponseMap.get(BlControllerConstants.REQUEST),
				(HttpServletResponse) requestAndResponseMap.get(BlControllerConstants.RESPONSE),
				getCategoryModelUrlResolver().resolve(category));
		...
		{
			if (category.getCode().startsWith(BlCoreConstants.NEW)
					|| BlCoreConstants.USED_NEW_ARRIVALS.equalsIgnoreCase(category.getCode()))
			{
		...
		populateModelAttributeForCategory(model, categorySearch, categoryCode, searchPageData, category, showMode,
				(HttpServletRequest) requestAndResponseMap.get(BlControllerConstants.REQUEST));
		populateRequiredDataForCategory(model, category, searchPageData,
				(HttpServletRequest) requestAndResponseMap.get(BlControllerConstants.REQUEST), searchQuery);
	 ...
	 */
	private String getDefaultSort(final CategoryModel category, final StringBuilder configParam, String searchQuery,
			final String categoryCode)
	{
		...
		else
		{
			...
			{
				searchQuery = String.valueOf(configParam.append(getConfigParameters(BlCoreConstants.DEFAULT_SORT_CODE))
						.append(getConfigParameters(BlCoreConstants.FACTED_CATEGORY_NAME)).append(categoryCode));
			}
		}
		return searchQuery;
	}
	...
	private void populateRequiredDataForCategory(final Model model, final CategoryModel category,
			...
			final HttpServletRequest request, final String searchQuery)
	{
	 ...
	 */
	private void populateModelAttributeForCategory(final Model model, final CategorySearchEvaluator categorySearch,
			final String categoryCode,
			final ProductCategorySearchPageData<SearchStateData, ProductData, CategoryData> searchPageData,
			final CategoryModel category, final ShowMode showMode, final HttpServletRequest request)
	{
	 ...
	 */
	private void populateModelForRentalAndUsedGearCategory(final Model model, final CategoryModel category,
	...
	private void setMetaData(final CategoryModel category, final Model model)
	{
		final String metaKeywords = MetaSanitizerUtil
				.sanitizeKeywords(category.getKeywords().stream().map(KeywordModel::getKeyword).collect(Collectors.toSet()));
	 ...
	 *           model
	 */
	private void checkNumberOfFacetSelected(
			final ProductCategorySearchPageData<SearchStateData, ProductData, CategoryData> searchPageData, final Model model)
	{
		...
		searchPageData.getFacets()
				.forEach(searchStateDataFacetData -> searchStateDataFacetData.getValues().forEach(searchStateDataFacetValueData -> {
	...
	private void updatePageTitleForRentalCategory(final CategoryModel category, final Model model)
	{
