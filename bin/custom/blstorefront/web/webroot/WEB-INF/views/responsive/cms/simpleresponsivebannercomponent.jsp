...
<li class="splide__slide simple-responsive">

<div class="simple-banner banner__component--responsive embed-responsive embed-responsive-16by9 ">
	...
	<c:choose>
		<c:when test="${empty simpleResponsiveBannerUrl || simpleResponsiveBannerUrl eq '#' || !ycommerce:validateUrlScheme(simpleResponsiveBannerUrl)}">
			<img class="js-responsive-image embed-responsive-item hero-image-slider" data-media='${fn:escapeXml(imagerDataJson)}' alt='${altTextHtml}' title='${altTextHtml}' style="">
		</c:when>
		<c:otherwise>
			<a href="${fn:escapeXml(simpleResponsiveBannerUrl)}">
				<img class="js-responsive-image embed-responsive-item hero-image-slider response" data-media='${fn:escapeXml(imagerDataJson)}' title='${altTextHtml}' alt='${altTextHtml}' style="">
			</a>
		</c:otherwise>
	</c:choose>
</div>
