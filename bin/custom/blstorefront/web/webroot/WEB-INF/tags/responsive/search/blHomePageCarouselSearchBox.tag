		...
		<ycommerce:testId code="header_search_input">
			...
			<c:if test="${positionAttribute == 'HomePageBannerSearchBoxSlot'}">
				<input type="text" id="js-site-search-input"
					...
					placeholder="<spring:theme code="text.hero.banner.searchbox.placeholder" />" name="text" value=""
					maxlength="100" data-options="${fn:escapeXml(optionsJson)}">
			...
			<c:if test="${positionAttribute == 'MobileHomePageBannerSearchBoxSlot'}">
				...
				<input type="text" id="mobile-litepicker"
					class="form-control d-inline-block" placeholder="<spring:theme code="text.hero.banner.searchbox.mobile.datepicker.placeholder" />">
			</c:if>
		</ycommerce:testId>
		<div class="input-group-append d-none d-md-block">
			<button class="btn btn-search btn-link js_search_button"
				type="submit"><spring:theme code="text.footer.subscription.button.search" /></button>
		</div>
	</div>
</form>