 ...
 */
package com.bl.Ordermanagement.integration;
...
import com.bl.Ordermanagement.integration.util.WorkflowUtil;
...
public class DeclineIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
  ...
  @Ignore
	public void shouldAutoReallocate_SingleEntry_SuccessReSourced__ToBusy_BanLocation() throws InterruptedException
	{
		...
		//refresh orderProcessModel until the newest value read
		while (!ProcessState.WAITING.equals(orderProcessModel.getProcessState())) {
  ...
  @Ignore
	public void shouldManualReallocatePartiallyAndReassignNewTaskAssignmentWorkFlowAfterPacking() throws InterruptedException
	{
		...
		while(commerceStockService.getStockLevelForProductAndBaseStore(products.Camera(), baseStores.NorthAmerica()) != Long.valueOf(2) || commerceStockService.getStockLevelForProductAndPointOfService(products.Camera(), pointsOfService.Boston()) != Long.valueOf(2) || commerceStockService
				.getStockLevelForProductAndPointOfService(products.Camera(), pointsOfService.Montreal_Downtown()) != Long.valueOf(0)) {
			modelService.refresh(order);
		}
