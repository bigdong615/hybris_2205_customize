...
public class SourcingUtil extends WorkflowUtil
{
			...
			final int readyConsignmentsSize, final int activeWorkflowsSize)
	{
		...
		while((order.getConsignments().stream()
				.filter(consignment -> consignment.getStatus().equals(ConsignmentStatus.CANCELLED)).collect(Collectors.toList()).size() != cancelledConsignmentsSize)){
		...
		while((order.getConsignments().stream()
				.filter(consignment -> consignment.getStatus().equals(ConsignmentStatus.READY)).collect(Collectors.toList()).size() != readyConsignmentsSize)){
			modelService.refresh(order);
		}
