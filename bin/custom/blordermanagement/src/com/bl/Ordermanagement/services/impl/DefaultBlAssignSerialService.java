...
public class DefaultBlAssignSerialService implements BlAssignSerialService {
  ...
  private ModelService modelService;
          ...
          if (sizeEqualToQuantity(context, result, results, entry, warehouse, forSaleFalseSerials,
            ...
            if (sizeEqualToQuantity(context, result, results, entry, warehouse, forSaleTrueSerials,
              ...
              if (sizeEqualToQuantity(context, result, results, entry, warehouse,
                ...
                boolean isEntrySourced = getUnAssignedAndFilteredSerials(context, result, results, entry, warehouse,
    ...
    return allEntrySourceComplete.stream().allMatch(isSourceComplete -> isSourceComplete.get() == true);
  }

  private boolean getUnAssignedAndFilteredSerials(SourcingContext context, SourcingResult result, Set<SourcingResult> results, AbstractOrderEntryModel entry, WarehouseModel warehouse,
  ...
  private Set<BlSerialProductModel> getUnAssignedSerials(Collection<BlSerialProductModel> allSerialProducts, List<String> assignedSerials) {
    return allSerialProducts.stream().filter(p -> assignedSerials.stream().anyMatch(s -> !s.equalsIgnoreCase(p.getCode()))).collect(Collectors.toSet());
  }

  private void createResultAndAssignSerials(SourcingContext context, SourcingResult result, Set<SourcingResult> results, AbstractOrderEntryModel entry, WarehouseModel warehouse,
      Set<BlSerialProductModel> serialProducts) {
    Map<AbstractOrderEntryModel, Long> allocation = MapUtils.isNotEmpty(result.getAllocation()) ? result.getAllocation() : new HashMap<AbstractOrderEntryModel, Long>();
    ...
    Set<String> serialProductCodes = serialProducts.stream().map(serial ->  serial.getCode()).collect(
        Collectors.toSet());
  ...
  private boolean sizeEqualToQuantity(SourcingContext context, SourcingResult result, Set<SourcingResult> results, AbstractOrderEntryModel entry, WarehouseModel warehouse, Set<BlSerialProductModel> serialProducts, Long fulfilledQuantity, List<String> assignedSerials) {
    if(serialProducts.size() == entry.getQuantity() && fulfilledQuantity <= entry.getQuantity()){
      ...
      assignedSerials.addAll(serialProducts.stream().map(p -> p.getCode()).collect(Collectors.toList()));
      return true;
    }
  ...
  private Set<BlSerialProductModel> getAllNonSaleAndNonBLConsignerSerials(Collection<BlSerialProductModel> serialProducts) {
    return serialProducts.stream().filter(serial -> serial.getOwnedBy() != "BL" && serial.getForSale() == false).collect(
        Collectors.toSet());
  }
  ...
  private Set<BlSerialProductModel> getAllForSaleSerials(Set<BlSerialProductModel> consignerSerials, boolean forSale) {
    return consignerSerials.stream().filter(serial -> serial.getForSale() == forSale).collect(Collectors.toSet());
  }

  private Set<BlSerialProductModel> getAllBLConsignerSerials(Collection<BlSerialProductModel> allSerialProducts) {
    return allSerialProducts.stream().filter(serial -> serial.getOwnedBy().equalsIgnoreCase("BL")).collect(Collectors.toSet());
  }
