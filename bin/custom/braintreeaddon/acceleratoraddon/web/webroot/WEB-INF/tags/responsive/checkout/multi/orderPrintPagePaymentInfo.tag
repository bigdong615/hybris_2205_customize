...
<%@ attribute name="showPaymentInfo" required="false"
...
<c:choose>
	<c:when test="${not empty paymentInfo}">
		...
		<c:if test="${empty cardName }">
			<c:choose>
				<c:when
					test="${fn:containsIgnoreCase(paymentInfo.subscriptionId, 'BrainTreePayPalExpress')}">
			...
			<div class="col-10 col-md-5">
				...
				<div class="row">
					<div class="col-6">
						<p class="body14">
							<c:if
								test="${fn:containsIgnoreCase(paymentInfo.subscriptionId, 'BrainTreePayPalExpress') == false }">
								<spring:theme code="text.review.page.payment.ending" />
								<br>
								<spring:theme code="text.review.page.payment.exp" />
								<br>
							</c:if>
