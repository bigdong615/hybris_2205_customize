...
public class BlDelayVoidOfTransactionJob extends AbstractJobPerformable<CronJobModel>
{
  ...
  {
    BlLogger.logFormatMessageInfo(LOG, Level.INFO, "Start performing BlDelayVoidOfTransactionJob...");
    try
    {
      getBlPaymentService().voidAuthTransaction();
      BlLogger.logFormatMessageInfo(LOG, Level.INFO, "BlDelayVoidOfTransactionJob finished successfully");
    }
    catch(final Exception ex)
    {
      BlLogger.logMessage(LOG, Level.ERROR, LogErrorCodeEnum.CRONJOB_ERROR.getCode(),
          "Error occurred while performing BlDelayVoidOfTransactionJob", ex);
