...
public class DefaultBlDeliveryModeService extends DefaultZoneDeliveryModeService implements BlDeliveryModeService
{
			...
			final String rentalEnd, final boolean payByCustomer)
	{
		...
		final Collection<ZoneDeliveryModeModel> upsZoneDeliveryModes = getShipToHomeDeliveryModesWithRentalDates(rentalStart,
				rentalEnd, String.valueOf(CarrierEnum.UPS), payByCustomer).stream()
	...
	public Collection<ZoneDeliveryModeModel> getShipToHomeDeliveryModesWithRentalDates(final String rentalStart,
			final String rentalEnd, final String carrier, final boolean payByCustomer)
	{
		...
		final int result = checkDateForRental(
				BlDateTimeUtils.getCurrentDateUsingCalendar(BlDeliveryModeLoggingConstants.ZONE_PST, new Date()), rentalStart);
		if (result >= BlInventoryScanLoggingConstants.TWO)
		{
			return getShipToHomeDeliveryModes(carrier, BlDeliveryModeLoggingConstants.DELIVERY_TYPE_STANDARD, null, payByCustomer);
		}
		else if (result == BlInventoryScanLoggingConstants.ONE)
		{
			...
			{
				return getShipToHomeDeliveryModes(carrier, BlDeliveryModeLoggingConstants.DELIVERY_TYPE_OVERNIGHT, pstCutOffTime,
						payByCustomer);
			}
		}
			...
			final String rentalEnd, final boolean payByCustomer)
	{
		...
		if (result >= BlInventoryScanLoggingConstants.TWO)
		{
			return getPartnerZoneUPSStoreDeliveryModes(BlDeliveryModeLoggingConstants.DELIVERY_TYPE_STANDARD, null, payByCustomer);
		}
		else if (result == BlInventoryScanLoggingConstants.ONE)
		{
			final DayOfWeek currentDayOfWeek = BlDateTimeUtils.getDayOfWeek(BlDeliveryModeLoggingConstants.ZONE_PST,
			...
			if (currentDayOfWeek.equals(DayOfWeek.SUNDAY) || currentDayOfWeek.equals(DayOfWeek.SATURDAY))
			{
				return getPartnerZoneUPSStoreDeliveryModes(BlDeliveryModeLoggingConstants.DELIVERY_TYPE_OVERNIGHT, null,
						payByCustomer);
			}
			else
			{
				return getPartnerZoneUPSStoreDeliveryModes(BlDeliveryModeLoggingConstants.DELIVERY_TYPE_OVERNIGHT, pstCutOffTime,
						payByCustomer);
			}
		}
		return Collections.emptyList();
			...
			final String rentalStart, final String rentalEnd, final boolean payByCustomer)
	{
		...
		if (CollectionUtils.isNotEmpty(blPickUpZoneDeliveryModeModels))
		{
			...
			return CollectionUtils.isNotEmpty(newBlPickUpZoneDeliveryModeModels)
	 ...
	 *
	 * @param blPickUpZoneDeliveryModeModels
	 *           collection to remove unwanted records
	 * @param result
	 *           business days difference
	 * @param pickUpZoneDeliveryModeModel
	 *           current model to check with condition
	 */
	private void checkDeliveryModeValidityOfTypePartner(
			final Collection<BlPickUpZoneDeliveryModeModel> blPickUpZoneDeliveryModeModels, final int result,
			final BlPickUpZoneDeliveryModeModel pickUpZoneDeliveryModeModel)
	{
		if (pickUpZoneDeliveryModeModel.isWarehousePickUp())
		{
			if (result == BlInventoryScanLoggingConstants.ZERO
					&& !BlDateTimeUtils.compareTimeWithCutOff(pickUpZoneDeliveryModeModel.getCutOffTime()))
			{
	 ...
	 */
	@Override
	public double getAmountForAppropriateZoneModel(final AbstractOrderModel order,
		...
		for (final ZoneDeliveryModeValueModel valueModel : zoneDeliveryModeModel.getValues())
		{
			...
			if (!valueModel.isFixedAmount())
			{
				...
				try
				{
					...
					return shippingCostModel != null ? Math.max(shippingCostModel, valueModel.getMinimum()) : valueModel.getMinimum();
				}
			...
			final Map<String, Double> calculatedValueMap)
	{
		...
		final ShippingCostModel shippingCostModel = getShippingCostForCalculatedDeliveryCost(
				String.valueOf(Math.max(calculatedValueMap.get(BlDeliveryModeLoggingConstants.TOTAL_WEIGHT),
						calculatedValueMap.get(BlDeliveryModeLoggingConstants.DIMENSIONAL_WEIGHT))),
		...
		try
		{
			...
			for (final AbstractOrderEntryModel entry : abstractOrderEntryModels)
			{
				...
				if (blSerialProduct != null)
				{
					...
					sumWidth = getSumWidth(sumWidth,
							((blSerialProduct.getWidth() != null ? blSerialProduct.getWidth() : BlInventoryScanLoggingConstants.ZERO)
			...
			final String rentalEnd)
	{
		...
		if (deliveryMode instanceof ZoneDeliveryModeModel
				&& checkCartEntriesAvailability(rentalStart, rentalEnd, (ZoneDeliveryModeModel) deliveryMode))
		{
			if (deliveryMode instanceof BlPickUpZoneDeliveryModeModel)
			{
				final BlPickUpZoneDeliveryModeModel zoneDeliveryModeModel = (BlPickUpZoneDeliveryModeModel) deliveryMode;
	...
	public boolean checkSFOrNYCPinCodeValidity(final String pinCode, final String deliveryType)
	{
		if (validateZip(pinCode))
		{
			try
			{
				final SameDayCityResData sameDayCityResData = getBlFedExSameDayService().getAvailability(
						getSameDayCityReqData(pinCode, BlDeliveryModeLoggingConstants.SF.equals(deliveryType) ? sf : nyc));
			...
			{
				BlLogger.logFormatMessageInfo(LOG, Level.DEBUG, "Exception in Checking same day fedex integration pinCode validity",
						e);
				return false;
			}
		}
	 ...
	 *
	 * @param pos
	 *           Point OF Service collection from warehouse
	 * @return zipCode
	 */
		...
		if (Objects.nonNull(deliveryModeModel))
		{
			...
			cartModel.getEntries().forEach(cartEntry -> {
				final StockResult stockForEntireDuration = getBlCommerceStockService()
						.getStockForEntireDuration(cartEntry.getProduct().getCode(), lWareHouses, rentalStartDate, rentalEndDate);
