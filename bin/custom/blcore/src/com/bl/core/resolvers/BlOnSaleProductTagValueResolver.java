...
/**
 * @author Manikandan
 * This Resolver created to populate onSale attribute for solr
 */
public class BlOnSaleProductTagValueResolver extends
  ...
  protected void addFieldValues(final InputDocument inputDocument,
    ...
    if(BooleanUtils.isTrue(blProductModel.getForSale()) && BooleanUtils.isTrue(blProductModel.getOnSale()) &&
        blProductModel.getSerialProducts().stream().anyMatch(
            blSerialProductModel -> BooleanUtils.isTrue(blSerialProductModel.getForSale()) && BooleanUtils.isTrue(blSerialProductModel
                .getOnSale()))) {
