...
/**
 * Adding this to have a case check for coupon id
 * for SQL Server Cloud Portal
 *
 * @author Ritika
 */
public class DefaultBlCouponDao extends DefaultCouponDao {
  ...
  public AbstractCouponModel findCouponById(String couponId) {
    String query = "SELECT {" + Item.PK + "} " + "FROM   {" + "AbstractCoupon" + "} " + "WHERE  {" + "couponId" + "} = ?couponId";
    ServicesUtil.validateParameterNotNull(couponId, "String couponId cannot be null");
    Map<String, String> params = Collections.singletonMap(BlCoreConstants.COUPON_ID, couponId);
  ...
  public SingleCodeCouponModel findSingleCodeCouponById(String couponId) {
    String singleCodeCouponQuery = "SELECT {" + Item.PK + "} " + "FROM   {" + "SingleCodeCoupon" + "} " + "WHERE  {" + "couponId" + "} = ?couponId";
    ...
    return (SingleCodeCouponModel)this.getFlexibleSearchService().searchUnique(new FlexibleSearchQuery(singleCodeCouponQuery, params));
  }
  ...
  public MultiCodeCouponModel findMultiCodeCouponById(String couponId) {
    ...
    return (MultiCodeCouponModel)this.getFlexibleSearchService().searchUnique(new FlexibleSearchQuery(multiCodeCouponQuery, params));
  }
}
