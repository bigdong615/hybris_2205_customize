...
public class HybrisDataController
{
	...
	public static void registerCustomDataClass(String id, HybrisCustomDataConverter customDataClass)
	{
	...
	static UDO extendUdo(String pageType, UDO udo)
	{
		if (customDataClassesMap != null)
		{
			...
			{
				HybrisCustomDataConverter customDataClass = customDataClassesMap.get(customDataClassKey);
				...
				switch (pageType)
				{
					...
					case "customer":
						...
						break;
					default:
						Map<String, HybrisCustomPageTypeCustomData> customPageMap = customDataClass.getHybrisCustomPageTypes();
	...
	static UDO getCustomPageUDO(String pageType, Map<String, HybrisCustomPageTypeCustomData> customData, UDO udo)
	{
		HybrisCustomPageTypeCustomData customDataRunnable = customData.get(pageType);
	...
	static UDO setupUDO(TealiumHelper tealiumHelper, PrebuiltUDOPageTypes pageType)
			throws UDODefinitionException, UDOUpdateException
	{
		...
		udo.setValue("page_type", pageNameString);
	...
	static TealiumHelper setupTealiumHelper() throws UDODefinitionException, UDOUpdateException
	{
	...
	public static String getSyncTag()
	{
		...
		try
		{
			String utagSyncEnabledString = Config.getParameter("tealiumiqaddon.utagSyncEnabled");
	...
	public static String getCustomPageScript(String pageType) throws UDODefinitionException, UDOUpdateException
	{
		String scriptString = "";
		try
		{
			...
			udo.setValue(TealiumHelper.HomePageUDO.PredefinedUDOFields.PAGE_TYPE, pageType);
	...
	public static String getProductPageScript() throws UDODefinitionException, UDOUpdateException
	{
		...
		try
		{
			...
			udo
					...
					.addArrayValues(TealiumHelper.ProductPageUDO.PredefinedUDOFields.PRODUCT_UNIT_PRICE, Arrays.asList(productPrice))
	...
	public static String getCartScript() throws UDODefinitionException, UDOUpdateException
	{
		...
		try
		{
			...
			List<String> productSubCategoryList = new ArrayList<>();
			...
			if (cartData != null)
			{
				...
				for (OrderEntryData entry : cartData.getEntries())
				{
					...
					productNameList.add(name != null ? name : "null");
					productQuantityList.add(quantity);
					productSkuList.add(sku);
					productUnitPriceList.add("");
					productSubCategoryList.add(((ProductData) entry.getProduct()).getCategories().stream().map(x -> x.getName())
							.collect(Collectors.joining("|")));
				}
			}

			udo.setValue(TealiumHelper.HomePageUDO.PredefinedUDOFields.PAGE_TYPE, "cart");
			if (productCategoryList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_CATEGORY, productCategoryList);
			}
			if (productListPriceList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_UNIT_PRICE, productListPriceList);
			}
			if (productNameList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_NAME, productNameList);
			}
			if (productQuantityList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_QUANTITY, productQuantityList);
			}
			if (productSkuList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_SKU, productSkuList);
			}
			if (productUnitPriceList.size() > 0)
			{
				udo.addArrayValues(TealiumHelper.CartPageUDO.PredefinedUDOFields.PRODUCT_LIST_PRICE, productUnitPriceList);
			}
			if (productSubCategoryList.size() > 0)
			{
				udo.addArrayValues("product_subcategory", productSubCategoryList);
			}
			udo.setValue("continue_to_checkout", "unknown");
	...
	public static String getOrderConfirmationScript() throws UDODefinitionException, UDOUpdateException
	{
		...
		try
		{
			...
			if (orderData != null)
			{
				...
				PriceData totalPriceData = (PriceData) orderData.getTotalPrice();
				PriceData deliveryCostData = (PriceData) orderData.getDeliveryCost();
				PriceData taxData = (PriceData) orderData.getTotalPrice();
				PriceData discountsData = (PriceData) orderData.getTotalDiscounts();
