...
public class ProductAvailCheckController extends DefaultWidgetController {
    ...
    public void getAvailableSerialData()
    {
        if (StringUtils.isBlank(productCode.getText()))
        {
            ...
            final Set<String> productCodeSets = new HashSet<String>(productCodeList);
            final String saparaterString = "    :";
            ...
            final LocalDate localStartDate = BlDateTimeUtils.convertStringDateToLocalDate(selectedFromDate, BlCoreConstants.SQL_DATE_FORMAT);
            final LocalDate localEndDate = BlDateTimeUtils.convertStringDateToLocalDate(selectedToDate, BlCoreConstants.SQL_DATE_FORMAT);
            final Date startDate = Date.from(localStartDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
            ...
            for (final WarehouseModel warehouse : warehouseModels) {
            ...
            final List<ProductAvailCheckToolData> productAvailCheckToolData = new ArrayList<>();
            if (CollectionUtils.isNotEmpty(finalStockLevels)) {
                ...
                final Collection<BlSerialProductModel> blSerialProducts = productDao.getBlSerialProductsForCodes(stockLevelsSerialWise.keySet());
...
//    System.out.println("After filter consigner serial i.e. owned by BL serial size:" + consignerSerial.size())
                    ...
                    availableSerialMap.values().forEach(serialProductList -> {
                        serialProductList.forEach(serialProduct -> {
                            ...
                            final ProductAvailCheckToolData productAvailCheckToolData1 = new ProductAvailCheckToolData();
     ...
     */
    public void getRenderData(final List<ProductAvailCheckToolData> productAvailCheckToolData)
    {
    ...
    public void setProductAvailCheckToolDataGrid(final Grid productAvailCheckToolDataGrid) {
