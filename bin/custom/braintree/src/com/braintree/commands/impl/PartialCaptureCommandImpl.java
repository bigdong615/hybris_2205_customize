...
public class PartialCaptureCommandImpl extends AbstractCommand implements BrainTreePartialCaptureCommand
{
	private static final Logger LOG = Logger.getLogger(PartialCaptureCommandImpl.class);
	...
	public BrainTreeSaleTransactionResult perform(final BrainTreeSaleTransactionRequest saleRequest)
	{
		...
		try
		{
			final Result<Transaction> result = getBraintreeGateway().transaction()
					.submitForPartialSettlement(saleRequest.getRequestId(), saleRequest.getTotalAmount());
			if (result.isSuccess())
			{
		...
		if (result.getErrors() != null)
		{
			...
			if (CollectionUtils.isNotEmpty(allDeepValidationErrors))
			{
				...
				getLoggingHandler().getLogger()
						.info(String.format("BT response error: %s, %s", validationError.getCode(), validationError.getMessage()));

				if (validationError.getCode() != null)
				{
					response.setErrorCode(validationError.getCode().code);
				}
	...
	private void addInVaultProperty(final TransactionOptionsRequest options)
	{
		if (Boolean.parseBoolean(getBrainTreeConfigService().getStoreInVault()))
		{
